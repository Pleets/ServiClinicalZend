...
class PatologiaTable
{
	...
	public function fetchAll()
	{
		...
		$sql = $select->from('hcneuro_patologia');
	...
	public function search($pac = "", $options = null)
	{
		...
		$sql = "SELECT b.cod_adm, c.est_adm, b.fecha_reg, a.cod_tip_doc, a.num_doc_pac,
				...
				LEFT JOIN hcneuro_servicios_salud AS f on a.cod_tip_doc = f.cod_tip_doc AND a.num_doc_pac = f.num_doc_pac AND a.num_fol = f.num_fol and f.cod_tip_his = 4
															and f.num_adm = b.cod_adm
				WHERE b.cod_tip_his = 4
	...
	public function getPatologia($id, $type, $folio)
	{
		$id = (string) $id;
	...
	public function isPatologia($id, $type, $folio)
	{
		$id = (string) $id;
		$rowset = $this->tableGateway->select(array('num_doc_pac' => $id, 'cod_tip_doc' => $type, 'num_fol' => $folio));
		$row = $rowset->current();
	...
	public function addPatologia(Patologia $history)
	{
		$data = array(
		...
		if ($this->isPatologia($history->num_doc_pac, $history->cod_tip_doc, $history->num_fol))
			throw new \Exception("Ya existe una patologia para el paciente $patient con número de folio $folio");
		else {
			try {
	...
	public function updatePatologia(Patologia $history)
	{
		$data = array(
			'material_estudio' => $history->material_estudio,
            'diagnostico_clinico' => $history->diagnostico_clinico,
	...
	public function deletePatologia($id, $type, $folio)
	{
		if ($this->isPatologia($id, $type, $folio))
