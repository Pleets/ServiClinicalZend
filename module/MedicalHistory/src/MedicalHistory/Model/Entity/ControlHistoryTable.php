...
class ControlHistoryTable
{
	...
	public function search($pac = "", $options = null)
	{
		...
		$sql = "SELECT b.cod_adm, c.est_adm, b.fecha_reg, a.cod_tip_doc, a.num_doc_pac, a.num_fol,
				CONCAT(d.pri_nom_pac, ' ', d.seg_nom_pac, ' ', d.pri_ape_pac, ' ', d.seg_ape_pac) AS nom_pac, nom_tip_doc,
				tip_evo, inf_sub, tas, tad, tam, fc, fr, tem, hal_exa, int_par, ana_con
				FROM hcneuro_histo_control AS a
	...
	public function addHistory(ControlHistory $history)
	{
		$data = array(
			'cod_tip_doc' => $history->cod_tip_doc,
			'num_doc_pac' => $history->num_doc_pac,
			'num_fol' => $history->num_fol,
			'tip_evo' => $history->tip_evo,
			'inf_sub' => $history->inf_sub,
			'tas' => $history->tas,
			'tad' => $history->tad,
			'tam' => $history->tam,
			'fc' => $history->fc,
			'fr' => $history->fr,
			'tem' => $history->tem,
			'hal_exa' => $history->hal_exa,
			'int_par' => $history->int_par,
	...
	public function updateHistory(ControlHistory $history)
	{
		$data = array(
			'tip_evo' => $history->tip_evo,
			'inf_sub' => $history->inf_sub,
